...
Redis.prototype.setValue=async function (key, value,option) {
    ...
    if (option.tree && !Array.isArray(value) && value !== null && typeof value === 'object') {
        ...
        let result = await self.client.set(fullKey, value);
...
Redis.prototype.clear = function (key, callback) {
  ...
  //
  this.client.srem(this.key.apply(this, path.concat(['keys'])), last, function (err) {
    //
    ...
    //
    self.client.smembers(nconf.key(fullKey, 'keys'), function (err, keys) {
      ...
      if (keys && keys.length > 0) {
        ...
        //
        self.client.del(fullKey, callback);
      }
    });
  });
};
...
Redis.prototype._addKeys = async function (key) {
    ...
    async function addKey(partial) {
        let index = path.indexOf(partial),
            base = [].concat(path.slice(0, index)),
            parent = self.key.apply(self, base.concat(['keys']));
        logger.debug('redis:sadd', parent);
        return await self.client.sadd(parent, partial);
    }

    let fns = [];
    path.forEach((partial) => {
        fns.push(addKey(partial));
    });
    return await Promise.all(fns);
